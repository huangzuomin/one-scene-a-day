...
- **积分月度管理（v1.7，2026-09-11 起生效）**：平台每月 2300 分 + 每日赠 60 分（约 30/31 天合计 4100/4160 分），月底清零。单价：2.0/720p = 6 分/秒（15s 片 = 90 分）；2.5/720p = 20 分/秒（15s 片 = 300 分）。目标＝月底余额趋近 0。规则：
  1. **基线夜**：默认 2.0/15s（90 分），沿用 v1.5 通道（主链路无模型行）
  2. **2.5 质量夜配额：每月 6 夜**（直连 Seedance_2.5 / 720p / 15s = 300 分/夜，走 §7 Plan B 命令并显式 `--resolution 720p`）。当月某夜是质量夜当且仅当：
     a. 触发优先：新标签首作 / 周报新提名首批 / recommend=高 且 potential≥8，且当月配额未用满 3 夜；
     b. 月末冲刺：22 日及以后，未用完的配额夜自动逐夜启用（保证清零前用完）；
     c. 回撤保护：当月 2.0 重拍已达 2 次 → 剩余配额夜减 1（防超支）。
  3. **台账估算**：每次提交把估算积分写入 budget-ledger（2.0=90、2.5=300），月节点 `months.YYYY-MM` 记录 `credits_spent_estimate` 与 `upgrades_used`
  4. **月末余额吸收器（需人工确认，不自动执行）**：最后夜预估余额仍 ≥300 且当月有已精选旧作时，可在收尾消息中提请一次 2.5/15s 精选重制；若余额 ≥600，可提请 2.5/30s 季精选加长重制（特别场合条款，PIPELINE v1.5）
...
这是一次全自动无人值守的创作任务：如需确认任何参数（画面比例、时长、风格等），一律按上述设定直接执行，不要等待确认。必须一次性整段生成全片，禁止拆分为多个镜头分别生成再拼接；全片总时长严格 15 秒，各镜头时长严格按分镜分配。"
```
...
*修订记录：2026-09-11 v1.7 积分月度管理上线：2.0/15s 基线 + 每月 6 夜 2.5/720p/15s 质量夜配额（触发优先+月末冲刺+重拍回撤），台账估算积分，月末余额吸收器留人工确认——目标月底清零前把积分用尽（v1.5 的『日常不用 2.5』条款就此修订，用户 2026-09-11 提出积分优化目标并给出单价）。*
